...
// examples/chat is 2 levels below the repo root.
 ...
 * realtime handler speaks — the same proven pattern as the repo's e2e app. It
 * runs the SSE handler inside Vite's dev server with no separate process.
 */
function realtimeServerPlugin(): Plugin {
  ...
  async function toWebRequest(req: IncomingMessage): Promise<Request> {
    ...
    return new Request(`http://localhost:5174${req.url ?? '/'}`, {
