/**
 * Tests for the @tanstack/vue-realtime composable behavior.
 *
 * These tests verify the underlying logic that each Vue composable encapsulates
 * using the core framework-agnostic API directly. They serve as a contract
 * specification for how each composable must behave, and can be run in a plain
 * Node.js environment without a DOM or Vue renderer.
 *
 * Vue composables return reactive `Ref<T>` values instead of plain `T`,
 * but the subscription logic, state transitions, and pub/sub contracts they
 * rely on are identical to those of the React hooks and Solid primitives —
 * this suite documents those contracts from the Vue package's perspective.
 *
 * For tests that require Vue's reactive owner context (createApp / mount),
 * a companion DOM suite using @vue/test-utils would be needed. This file
 * focuses on the pure client-logic contracts that every Vue composable depends on.
 */
...
describe('useSyncedSet (OR-Set CRDT) contract', () => {
  it('add inserts element', () => {
    ...
    expect(orValues(state)).toContain('vue')
  })

  it('remove deletes element', () => {
    let state = orAdd({ entries: [] }, 'vue')
    state = orRemove(state, 'vue')
    expect(orValues(state)).not.toContain('vue')
  })

  it('concurrent add wins over concurrent remove (OR-Set semantics)', () => {
    const base = orAdd({ entries: [] }, 'vue')
    const clientA = { ...base } // keeps add
    const clientB = orRemove(base, 'vue') // concurrent remove

    const merged = mergeOr(clientA, clientB)
    // Add always wins over concurrent remove in OR-Set
    expect(orValues(merged)).toContain('vue')
  })
...
// ---------------------------------------------------------------------------
// useStoreRef — @tanstack/store → Vue ref bridge contract
//
// useStoreRef is an @internal utility that bridges a @tanstack/store Store to
// a Vue reactive ref. Full reactivity tests (automatic updates when the store
// changes) require a Vue component setup context. The tests below verify the
// store-side contract: that the correct initial value is returned and that
// the store's subscribe mechanism fires correctly so the ref can update.
// ---------------------------------------------------------------------------

describe('useStoreRef — underlying store contract', () => {
  it('store.state reflects the initial value used to initialise the ref', () => {
    const { client } = makeClient()
    // useStoreRef initialises the ref with selector(store.state)
  ...
  it('selector is applied to project a sub-slice of store state', () => {
    const { client } = makeClient()
    // useStoreRef uses selector(store.state) for the initial value
