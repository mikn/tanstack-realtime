...
/**
 * Provides a `RealtimeClient` to the component tree via Solid context.
 * All primitives from `@tanstack/solid-realtime` (`useRealtime`, `usePresence`,
 ...
 * }
 */
export const RealtimeProvider: ParentComponent<RealtimeProviderProps> = (
  props,
) => {
  createEffect(() => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    const status = createStoreSignal(props.client.store, (s) => s.status)
  ...
  return (
    <RealtimeContext.Provider value={props.client}>
      {props.children}
