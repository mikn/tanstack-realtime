...
import { createServerStream, serializeKey } from '@realtimejs/core'
import type { PublishFn, QueryKey, ServerStream } from '@realtimejs/core'
import type { SseHandlerOptions } from '@realtimejs/adapter-sse'
 ...
 * application. It composes `createSseHandler` (from `@realtimejs/adapter-sse`)
 ...
 * ```ts
 * // app/server/realtime.ts
 * import { createStartHandler, type PublishBackend } from '@realtimejs/preset-start'
 *
...
export function createStartHandler(
  ...
  // `onChannelEmpty` is forwarded straight through to the SSE handler. The
  // reactive engine (from `@realtimejs/reactive-drizzle`) provides an
  // `onChannelEmpty` callback that unregisters channels; users compose it here.
